...
struct command_lookup cmd_lookup_table[] = {
    {"fg(", fg, 3},       {"bg(", bg, 3},    {"i(", icon, 2},
    {"r(", rect, 2},      {"ro(", recto, 3}, {"c(", circle, 2},
    {"co(", circleo, 3},  {"p(", pos, 2},    {"pa(", abspos, 3},
    {"tw(", titlewin, 3}, {"ib(", ibg, 3},   {"fn(", fn, 3},
    {"ca(", ca, 3},       {"ba(", ba, 3},    {0, 0, 0}};
...
{
    /* in case text == "", XftTextExtentsUtf8 does not set xOff to zero */
    if(!len)
        return 0;
#ifndef DZEN_XFT
    XRectangle r;
...
#else
    XftTextExtentsUtf8(dzen.dpy, dzen.font.xftfont, (unsigned const char *)text,
                       strlen(text), dzen.font.extents);
...
void drawtext(const char *text, int reverse, int line, int align)
{
    if (!reverse) {
        ...
        XFillRectangle(dzen.dpy, dzen.slave_win.drawable[line], dzen.gc, 0, 0,
                       dzen.w, dzen.h);
        XSetForeground(dzen.dpy, dzen.gc, dzen.norm[ColFG]);
    }
    else {
        XSetForeground(dzen.dpy, dzen.rgc, dzen.norm[ColFG]);
        XFillRectangle(dzen.dpy, dzen.slave_win.drawable[line], dzen.rgc, 0, 0,
                       dzen.w, dzen.h);
...
void setfont(const char *fontstr)
{
    ...
    XftTextExtentsUtf8(dzen.dpy, dzen.font.xftfont,
...
static void setcolor(Drawable *pm, int x, int width, long tfg, long tbg,
                     int reverse, int nobg)
{
    if (nobg)
        return;
...
static int search_icon_cache(const char *name)
{
    int i;
...
char *parse_line(const char *line, int lnr, int align, int reverse, int nodraw)
{
    ...
    /* parse line and render text */
        ...
        if (lnr != -1) {
            pm = XCreatePixmap(dzen.dpy,
                               RootWindow(dzen.dpy, DefaultScreen(dzen.dpy)),
        ...
        if (lnr != -1 &&
            (lnr + dzen.slave_win.first_line_vis >= dzen.slave_win.tcnt)) {
            XCopyArea(dzen.dpy, pm, dzen.slave_win.drawable[lnr], dzen.gc, 0, 0,
                      px, dzen.line_height, xorig[LNR2WINDOW(lnr)], 0);
    ...
    while (1) {
        if (*linep == ESC_CHAR || *linep == '\0') {
            ...
            if (nodraw) {
                ...
                if (t != -1 && tval) {
                    switch (t) {
                        case icon:
                            if (MAX_ICON_CACHE &&
                                (ip = search_icon_cache(tval)) != -1) {
                                int y;
                                XCopyArea(
                                    dzen.dpy, icons[ip].p, pm, dzen.tgc, 0, 0,
                                    icons[ip].w, icons[ip].h, px,
                                    y = (set_posy ? py
                                                  : (dzen.line_height >=
                                                             (signed)icons[ip].h
                                                         ? (dzen.line_height -
                                ...
                                if (XReadBitmapFile(dzen.dpy, pm, tval, &bm_w,
                                    ...
                                    setcolor(&pm, px, bm_w, lastfg, lastbg,
                                             reverse, nobg);

                                    XCopyPlane(
                                        dzen.dpy, bm, pm, dzen.tgc, 0, 0, bm_w,
                                        bm_h, px,
                                        y = (set_posy
                                                 ? py
                                                 : (dzen.line_height >=
...
#ifdef DZEN_XPM
                                else if (XpmReadFileToPixmap(
                                             dzen.dpy, dzen.title_win.win, tval,
                                             &xpm_pm, NULL,
                                             &xpma) == XpmSuccess) {
                                    setcolor(&pm, px, xpma.width, lastfg,
                                             lastbg, reverse, nobg);

                                    if (MAX_ICON_CACHE)
                                        cache_icon(tval, xpm_pm, xpma.width,
                                                   xpma.height);

                                    XCopyArea(
                                        dzen.dpy, xpm_pm, pm, dzen.tgc, 0, 0,
                                        xpma.width, xpma.height, px,
                                        y = (set_posy
                                                 ? py
                                                 : (dzen.line_height >=
                                                            (int)xpma.height
                        ...
                        case rect:
                            ...
                            recth = recth > dzen.line_height ? dzen.line_height
                                                             : recth;
                            if (set_posy)
                                py += recty;
                            recty =
                                recty == 0
                                    ? (dzen.line_height - recth) / 2
                            ...
                            XFillRectangle(
                                dzen.dpy, pm, dzen.tgc, px,
                                set_posy
                                    ? py
                                    : ((int)recty < 0 ? dzen.line_height + recty
                                                      : recty),
                                rectw, recth);
                        ...
                        case circle:
                            ...
                            XFillArc(
                                dzen.dpy, pm, dzen.tgc, px,
                                set_posy ? py : (dzen.line_height - rectw) / 2,
                                rectw, rectw, 90 * 64,
                        ...
                        case bg:
                            lastbg = tval[0] ? (unsigned)getcolor(tval)
                                             : dzen.norm[ColBG];
#ifdef DZEN_XFT
                            if (xftcs_bgf)
                                free(xftcs_bg);
                        ...
                        case fg:
                            lastfg = tval[0] ? (unsigned)getcolor(tval)
                                             : dzen.norm[ColFG];
                        ...
                        case fn:
                            if (tval[0]) {
                                ...
                                if (!strncmp(tval, "dfnt", 4)) {
                                    ...
                                    if (!cur_fnt->set) {
                                        ...
                                        XChangeGC(dzen.dpy, dzen.tgc, GCFont,
                                                  &gcv);
                                    }
                                }
                                else
                            ...
                            py = set_posy
                                     ? py
                                     : (dzen.line_height - cur_fnt->height) / 2;
                            font_was_set = 1;
                            break;
                        case ca:;  // nop to keep gcc happy
                            ...
                            if (tval[0]) {
                                click_a *area =
                                    &((*w).sens_areas[(*w).sens_areas_cnt]);
                                if ((*w).sens_areas_cnt < MAX_CLICKABLE_AREAS) {
                                    get_sens_area(tval, &(*area).button,
                                                  (*area).cmd);
                                    (*area).start_x = px;
                                    (*area).start_y = py;
                                    (*area).end_y = py;
                        ...
                        case ba:
                            if (tval[0])
                                get_block_align_vals(tval, &block_align,
                                                     &block_width);
                            else
                ...
                if (block_align != -1 && !nobg) {
                    ...
                    XFillRectangle(dzen.dpy, pm, dzen.tgc, px, 0, block_width,
                                   dzen.line_height);
                }
...
#ifndef DZEN_XFT
                if (cur_fnt->set)
                    XmbDrawString(dzen.dpy, pm, cur_fnt->set, dzen.tgc, px,
                                  py + cur_fnt->ascent, lbuf, strlen(lbuf));
                else
                    XDrawString(dzen.dpy, pm, dzen.tgc, px,
                                py + dzen.font.ascent, lbuf, strlen(lbuf));
#else
                if (reverse) {
                    XftColorAllocName(dzen.dpy,
            ...
            // fprintf(stderr, "linep: \"%s\"\n", linep);
            // fprintf(stderr, "tval: (%lu), \"%s\"\n", strlen(tval), tval);
            // fprintf(stderr, "t: %d\n", t);
            // fprintf(stderr, "next_pos: %d\n", next_pos);
            linep += next_pos;
    ...
    if (!nodraw) {
            ...
            switch (dzen.title_win.expand) {
                case left:
                    ...
                    otx = dzen.title_win.x_right_corner - i > dzen.title_win.x
                              ? dzen.title_win.x_right_corner - i
                              : dzen.title_win.x;
                    XMoveResizeWindow(dzen.dpy, dzen.title_win.win, otx,
                                      dzen.title_win.y, px, dzen.line_height);
                    break;
                case right:
                    XResizeWindow(dzen.dpy, dzen.title_win.win, px,
                                  dzen.line_height);
                    break;
            }
        }
        ...
        if (lnr != -1) {
            XCopyArea(dzen.dpy, pm, dzen.slave_win.drawable[lnr], dzen.gc, 0, 0,
                      dzen.w, dzen.line_height, xorig[LNR2WINDOW(lnr)], 0);
        }
        else {
            XCopyArea(dzen.dpy, pm, dzen.title_win.drawable, dzen.gc, 0, 0,
                      dzen.w, dzen.line_height, xorig[LNR2WINDOW(lnr)], 0);
        }
        XFreePixmap(dzen.dpy, pm);
...
#ifdef DZEN_XPM
        if (free_xpm_attrib) {
            XFreeColors(dzen.dpy, xpma.colormap, xpma.pixels, xpma.npixels,
                        xpma.depth);
...
void drawheader(const char *text)
{
    if (parse_non_drawing_commands((char *)text)) {
        if (text) {
            ...
            XFillRectangle(dzen.dpy, dzen.title_win.drawable, dzen.rgc, 0, 0,
                           dzen.w, dzen.h);
    ...
    XCopyArea(dzen.dpy, dzen.title_win.drawable, dzen.title_win.win, dzen.gc, 0,
              0, dzen.title_win.width, dzen.line_height, 0, 0);
}
